...
public class AdminPage {
	...
	// "this" here representing current object that is being called
	public AdminPage() {
	...
	public void addNationality(String nationality) {
		List<WebElement> nationalityOptions = nationalitiesButton
		...
		for (WebElement option : nationalityOptions) {
			if (option.getText().equals("Aussila")) {
	...
	public void deleteNationality(String nationality) {
		...
		for (WebElement option : nationalityOptions) {
			if (option.getText().equals(nationality)) {
				...
				BaseClass.getDriver().switchTo().window(mainWindowHandle);
